...
const Profile = () => {
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      setProfileData(prev => ({
        ...prev,
        posts: prev.posts.map(p => p._id === postId ? { ...p, likes: likes, isLikedByMe: isLiked } : p)
      }));
    } catch (error) {
        ...
        {viewerPost && (
          ...
          >
            ...
            >
              ...
              {/* ===== КОММЕНТАРИИ (ПОЛНЫЙ ФУНКЦИОНАЛ ВОССТАНОВЛЕН) ===== */}
              <AnimatePresence>
                {isCommentsOpen && (
                  <motion.div
                    initial={{ x: '100%' }}
                        ...
                        return (
                          <div key={comment._id} className="border-b border-white/5 pb-4">
                            ...
                            <div className="flex items-center gap-4 ml-11 mt-1">
                              ...
                              {/* РЕДАКТИРОВАТЬ/УДАЛИТЬ (ТОЛЬКО ДЛЯ АВТОРА) */}
              ...
              {isOwnProfile ? (
                ...
                <>
                  <button 
                    ...
                    className={`px-6 py-2 rounded-xl font-semibold transition ${isFollowing ? 'bg-white/10 text-white hover:bg-white/20' : 'bg-accent text-white hover:bg-accent/80'}`}
                  >
