...
const CreateStory = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!title.trim()) {
      alert('Пожалуйста, введите название истории!');
      return;
    }
    if (!selectedFile) {
  ...
  return (
    <div className="min-h-screen bg-dark flex flex-col items-center justify-center p-6">
      ...
      >
        <h2 className="text-2xl font-bold text-white mb-6 text-center">Создать историю</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            type="text"
            placeholder="Название истории (обязательно)"
            value={title}
